...
def write_partition_to_disk(arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, num_record, block_start_idx, block_end_idx, comp_data_dir):
...
def delete_tuple_in_disk(arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, num_record, num_col, block_start_idx, block_end_idx, comp_data_dir, data_op, key, data_ori_dtype):
    ...
    for block_idx in range(block_start_idx, block_end_idx):
        ...
        try:
            ...
            continue
        curr_decomp_block = np.frombuffer(block_bytes, dtype=data_ori_dtype)
...
def measure_latency(df, data_ori, task_name, sample_size, 
    ...
    if generate_file:
        ...
        print(data_partition_idx.nbytes/1024/1024)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads_to_generate_file) as executor:
            ...
            futures = [executor.submit(write_partition_to_disk, arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, data_partition_idx.shape[0], i*num_partition_per_core, (i+1)*num_partition_per_core, comp_data_dir) for i in range(num_threads_to_generate_file)]
...
def measure_latency_data_update(df, data_ori, data_change, data_op, task_name, sample_size, 
    ...
    for _ in tqdm(range(num_loop)):
        ...
        if data_op == 'Insert' or data_op == 'Update':
            ...
            print("debug:", x_start, x_end, data_partition_idx_start,num_partition_per_core)
