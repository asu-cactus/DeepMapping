...
def write_partition_to_disk(arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, num_record, block_start_idx, block_end_idx, key, comp_data_dir):
...
def update_partition_to_disk(arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, num_record, block_start_idx, block_end_idx, comp_data_dir, data_op, key):
    # Open the shared memory array
    arr_shm = shm.SharedMemory(name=arr_shm_name)
    # Access the shared array as a NumPy array
    data_ori = np.ndarray((num_record,), dtype=arr_shm_dtypes, buffer=arr_shm.buf)
    ...
    for block_idx in range(block_start_idx, block_end_idx):
        ...
        try:
            ...
            for data_idx in range(len(data_part)):
                ...
                with ndb_utils.get_temp_file_path() as temp_path:
                    ndb_utils.save_hashtable_to_disk(temp_path, data_part_hash_table)
            continue
        ...
        with ndb_utils.get_temp_file_path() as temp_path:
            ndb_utils.save_hashtable_to_disk(temp_path, curr_hash_table)
        # total_size += sys.getsizeof(data_bytes)
        if enable_progress_bar:
            progress_bar.update(1)
    return total_size

def delete_tuple_in_disk(arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, num_record, num_col, block_start_idx, block_end_idx, comp_data_dir, data_op, key, data_ori_dtype):
    # Open the shared memory array
    arr_shm = shm.SharedMemory(name=arr_shm_name)
    # Access the shared array as a NumPy array
...
def measure_latency(df, data_ori, task_name, sample_size, 
    ...
    if generate_file:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads_to_generate_file) as executor:
            ...
            futures = [executor.submit(write_partition_to_disk, arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, data_partition_idx.shape[0], i*num_partition_per_core, (i+1)*num_partition_per_core, key, comp_data_dir) for i in range(num_threads_to_generate_file)]
...
def measure_latency_range_query(df, data_ori, task_name, range_window, 
                    generate_file=True,
                    num_loop=10, num_query=5, search_algo='binary', block_size=1024*1024, **kwargs):
    """Measure the end-end latency of data query
    ...
    # generate file
    if generate_file:
        raise ValueError("Please generate file with point data query function")
    else:
        exp_data_dict = ndb_utils.load_obj_from_disk_with_pickle(os.path.join(comp_data_dir, 'extra_meta.data'))
        num_record_per_part = exp_data_dict['num_record_per_part']
        data_ori_size = exp_data_dict['data_ori_size']
        ...
        x_start = exp_data_dict['x_start']
        x_end = exp_data_dict['x_end']
        list_range_query = ndb_utils.load_obj_from_disk_with_pickle(os.path.join(root_path, task_name, 'range_query_{}.data'.format(range_window)))
    ...
    for _ in tqdm(range(num_loop)):  
        ...
        for query_idx in range(num_query):
            query_start_pos, actual_query_range = list_range_query[query_idx]
            timer_total.tic()
            timer_sort.tic()
            query_stop_pos = query_start_pos + actual_query_range
            # query_part_start = int((query_start_pos - x_start) // num_record_per_part)
            # query_part_end = int((query_stop_pos - x_start) // num_record_per_part)
            # num_partition_to_decompress = query_part_end - query_part_start + 1
            # sample_index_sorted = np.sort(sample_index)
            # sample_index_argsort = np.argsort(sample_index)
            sample_index_sorted = np.arange(query_start_pos, query_stop_pos)
            sample_index_partition = (sample_index_sorted - x_start) // num_record_per_part
            sample_index_partition = sample_index_partition.astype(np.int32)
            t_sort += timer_sort.toc()
            result = np.recarray((actual_query_range,), dtype=data_ori.dtype)
            result_idx = 0
            
            for idx in range(actual_query_range):
                query_key = sample_index_sorted[idx]
                query_key_index_in_old = idx
                timer_locate_part.tic()        
                part_idx = sample_index_partition[idx]
                t_locate_part += timer_locate_part.toc()
                timer_decomp.tic()
    ...
    arr_latency = np.array((data_ori_size, data_comp_size, actual_query_range, peak_memory/1024/1024, t_sort / num_loop, 
    0, 0, t_locate_part / num_loop, t_decomp / num_loop, t_build_index / num_loop,
    t_lookup / num_loop, 0, 0, t_total / num_loop, num_decomp, count_nonexist)).T
...
def measure_latency_data_update(df, data_ori, data_change, data_op, task_name, sample_size, 
    ...
    for _ in tqdm(range(num_loop)):
        ...
        if data_op == 'Insert' or data_op == 'Update':
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads_to_generate_file) as executor:
                # Launch threads to process the numbers
                futures = [executor.submit(update_partition_to_disk, arr_shm_name, arr_shm_dtypes, arr_shm_name1, arr_shm_dtypes1, data_partition_idx.shape[0], data_partition_idx_start+i*num_partition_per_core, data_partition_idx_start+(i+1)*num_partition_per_core, comp_data_dir, data_op, key) for i in range(num_threads_to_generate_file)]
                # Retrieve the results from each thread
                for future in concurrent.futures.as_completed(futures):
                    future.result()
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads_to_generate_file) as executor:
                ...
                futures = [executor.submit(delete_tuple_in_disk, arr_shm_name2, arr_shm_dtypes2,
                                        arr_shm_name3, arr_shm_dtypes3, data_idx_to_delete.shape[0], data_idx_to_delete.shape[1], data_partition_idx_start+i*num_partition_per_core, data_partition_idx_start+(i+1)*num_partition_per_core, comp_data_dir, data_op, key, data_ori.dtype) for i in range(num_threads_to_generate_file)]
                # Retrieve the results from each thread
                for future in concurrent.futures.as_completed(futures):
                    future.result()
            arr_shm2.close()
